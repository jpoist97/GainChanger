...
export async function fetchWorkouts() {
   ...
   workoutSnapshot.forEach((doc) => {
      const { exercises, name, lastPerformed, muscleGroups } = doc.data();
      const dateDiff = differenceInCalendarDays(
         new Date(),
         parse(lastPerformed, 'yyyy-MM-dd', new Date())
      );
...
export async function purgeWorkoutFromCycles(workoutId, cycleIds) {
   ...
   const cycleRefs = cycleIds.map((cycleId) =>
      userRef.collection('cycles').doc(cycleId).get()
   );
   ...
   cycleDocuments.forEach((cycleDoc) => {
      ...
      const filteredWorkouts = data.workoutIds.filter(
         (filterWorkoutId) => filterWorkoutId !== workoutId
      );
...
export async function getCycleIdsContainingWorkout(workoutId) {
   ...
   const querySnapshot = await userRef
      .collection('cycles')
      .where('workoutIds', 'array-contains', workoutId)
      .get();
...
export async function retrieveExerciseRecords(exerciseId) {
   ...
   const querySnapshot = await execiseRecordsRef
...
export async function postExerciseRecords(exerciseRecords) {
   ...
   const dbPostPromises = exerciseRecords.map((record) =>
      execiseRecordsRef.add(record)
   );
