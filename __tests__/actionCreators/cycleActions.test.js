...
const cycles = [
   {
      ...
      workoutIds: ['workout-id-a', 'workout-id-b'],
   },
   {
      name: 'Upper/Legs split',
      id: 'cycle-id-b',
      workoutIds: ['workout-id-c', 'workout-id-d'],
   },
];
...
describe('cycleActions tests', () => {
   describe('initializeCycles tests', () => {
      it('should dispatch INITIALIZE_CYCLES with fetchCycles result', async () => {
         ...
         // Act
         const thunkFunction = cycleActions.initializeCycles(
            selectedCycleId,
            selectedCycleIndex
         );
   ...
   describe('incrementSelectedCycleIndex tests', () => {
      it('should dispatch SET_SELECTED_CYCLE_INDEX with incremented cycle index', () => {
         ...
         // Act
         const thunkFunction = cycleActions.incrementSelectedCycleIndex(
            prevIndex,
            cycleLength
         );
      ...
      it('should call updateSelectedCycleIndex to update firestore', () => {
         ...
         // Assert
         expect(api.updateSelectedCycleIndex).toHaveBeenCalledWith(
            prevIndex + 1
         );
      });
   ...
   describe('decrementSelectedCycleIndex tests', () => {
      it('should dispatch SET_SELECTED_CYCLE_INDEX with decremented cycle index', () => {
         ...
         // Act
         const thunkFunction = cycleActions.decrementSelectedCycleIndex(
            prevIndex,
            cycleLength
         );
         thunkFunction(mockDispatch);
