...
const ExerciseList = ({
   ...
   const [dataState, setDataState] = useState({
      isSortByMuscleGroup: false,
      filteredDataSource: [],
      masterDataSource: parsedItemsName,
   });
   ...
   const renderHeader = ({ section }) => (
      <SectionHeader backgroundColor={COLORS[colorTheme][0]}>
         {section.title}
      </SectionHeader>
   );
   ...
   const handleSortByNamePress = () => {
      setDataState({
         isSortByMuscleGroup: false,
         filteredDataSource: parsedItemsName,
         masterDataSource: parsedItemsName,
      });
   };
   ...
   const handleSortByMuscleGroupPress = () => {
      setDataState({
         isSortByMuscleGroup: true,
         filteredDataSource: parsedItemsMuscleGroups,
         masterDataSource: parsedItemsMuscleGroups,
      });
   };
   ...
   const searchFilterFunction = (text) => {
      if (text) {
         ...
         setDataState({
            ...dataState,
            filteredDataSource: dataState.masterDataSource,
         });
         setSearch(text);
      }
   };
   ...
   const handleUnfilteredCardPress = (item, index) => {
      ...
      const selectedGroup = dataState.isSortByMuscleGroup
         ? item.muscleGroups
         : item.name[0];
   ...
   const renderCard = ({ item, index }) => (
      <ExerciseItem
         ...
         onPress={() =>
            search
               ? handleFilteredCardPress(item, index)
               : handleUnfilteredCardPress(item, index)
         }
      />
   );
   ...
   return (
      <SafeAreaView style={{ height: '100%' }}>
         ...
         >
            <Buttontext>Add ({exerciseCount})</Buttontext>
         </ButtonContainer>
               ...
               options={[
                  {
                     icon: 'ALPHABET',
                     text: 'Sort By Name',
                     onPress: handleSortByNamePress,
                  },
                  {
                     icon: 'RUNNING',
                     text: 'Sort By Muscle Group',
                     onPress: handleSortByMuscleGroupPress,
                  },
               ]}
               triggerSize={28}
            />
