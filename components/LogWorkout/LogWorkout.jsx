...
const LogWorkout = (props) => {
   const {
      route: {
         params: { workoutId, isSelectedCycle, cycleLength },
      },
   } = props;
   ...
   const exercises = selectedWorkout.exercises.map((exerciseObj, index) => {
      const matchingExercise = _.find(
         exerciseStore,
         (exercise) => exerciseObj.exerciseId === exercise.id
      );
   ...
   const updatePastWorkoutDates = (completedDate) => {
      userRef.update({
         pastWorkoutDates:
            firebase.firestore.FieldValue.arrayUnion(completedDate),
      });
   ...
   const sendWorkoutLogToDB = () => {
      ...
      const newWorkoutLog = {
         ...
         exercises: exerciseState.map((exercise) => ({
            ...
            sets: exercise.sets.map((set) => {
               const parsedSet = {
                  weight:
                     set.prevWeight === 'n/a' && set.weight === ''
                        ? null
                        : parseFloat(set.weight) || parseFloat(set.prevWeight),
               };
               _.set(
                  parsedSet,
                  exercise.type === 'REPS' ? ['reps'] : ['time'],
                  parseInt(set.duration) || parseInt(set.prevDuration)
               );
               return parsedSet;
            }),
         })),
      ...
      newWorkoutLog.exercises.forEach((exercise) => {
         dispatch(
            actions.records.addWorkoutRecord(exercise, newWorkoutLog.date)
         );
      });
      userRef.collection('workouts').doc(workoutId).update(newWorkoutDoc); // updates workout doc
      dispatch(
         actions.workouts.updateWorkoutExercises(
            workoutId,
            newWorkoutLog.exercises
         )
      ); // rerenders workout to show update prev details
   ...
   const updateUserProgress = () => {
      ...
      exerciseState.forEach((exerciseDetails) => {
         ...
         exerciseDetails.sets.forEach((setDetails) => {
            const weight = parseFloat(
               setDetails.weight || setDetails.prevWeight || '0'
            );
            const duration = parseInt(
               setDetails.duration || setDetails.prevDuration
            );
      ...
      dispatch(
         actions.progress.updateProgressStats(
            totalWeightLifted,
            profileStats.totalWorkoutsPerformed + 1,
         ...
         <StyledFinishButton
            onPress={() => {
               ...
               if (!completed) {
                  ...
                  if (isSelectedCycle) {
                     dispatch(
                        actions.cycles.incrementSelectedCycleIndex(
                           cycleIdx,
                           cycleLength
                        )
                     );
