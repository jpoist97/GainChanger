...
const CalendarView = () => {
   ...
   React.useEffect(() => {
      setMarkedDates(
         createCalendarMarkedDates(pastWorkoutDates, COLORS[colorTheme][0])
      );
      setExercises([]);
   ...
   const getDateRecords = async (user, currentDate) => {
      ...
      if (workoutRecords[currentDate]) {
         ...
         const workoutRecordSnapshot = await recordsRef
   ...
   const onDayPress = (date) => {
      const allDates = createCalendarMarkedDates(
         pastWorkoutDates,
         COLORS[colorTheme][0]
      );
      allDates[date.dateString] = {
         marked: allDates[date.dateString]
            ? allDates[date.dateString].marked
            : false,
         selectedColor: COLORS[colorTheme][0],
         selected: true,
      };
         ...
         {showWorkout ? (
            <FlatList
               ...
               keyExtractor={(item, index) =>
                  item.name + item.date + index.toString()
               }
               renderItem={({ item, index }) => {
                  ...
                  return (
                     <CalendarWorkoutCard
                        color={
                           COLORS[colorTheme][
                              index % (COLORS[colorTheme].length - 1)
                           ]
                        }
