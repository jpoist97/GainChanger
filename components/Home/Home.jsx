...
const Home = () => {
   ...
   const filterWorkoutListForDisplay = () => {
      ...
      workoutList.forEach((workout) => {
         if (
            Number.isNaN(workout.lastPerformed) ||
            workout.lastPerformed === 'n/a'
         ) {
   ...
   const selectedCycle =
      ...
      _.find(cycles.cycles, (cycle) => cycle.id === cycles.selectedCycleId);
   let cycleDetails;
   if (!cycles.selectedCycleDetails && selectedCycle) {
      cycleDetails = selectedCycle.workouts.map((workoutId) =>
   ...
   return (
      <SafeAreaView>
         ...
         >
            ...
            <Animatable.Text animation="bounceIn" iterationCount={1}>
               <WelcomeTitle numberOfLines={1}>{welcomeName}!</WelcomeTitle>
            </Animatable.Text>
         </View>
         <View style={{ height: '50%', marginBottom: '25%' }}>
            <CurrentCycle
               name={
                  cycleDetails && cycleDetails[cycles.selectedCycleIndex].name
               }
               subtext={
                  cycleDetails &&
                  cycleDetails[cycles.selectedCycleIndex].muscleGroups
               }
               color={
                  cycleDetails &&
                  COLORS[colorTheme][
                     cycles.selectedCycleIndex % (COLORS[colorTheme].length - 1)
                  ]
               }
               leftPress={() => {
                  dispatch(
                     actions.cycles.decrementSelectedCycleIndex(
               ...
               id={
                  cycleDetails &&
                  selectedCycle.workouts[cycles.selectedCycleIndex]
               }
