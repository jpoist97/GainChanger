...
export default () => {
   ...
   const exerciseRecords = useSelector(
      (state) => state.progress.exerciseRecords
   );
   const loading = useSelector((state) => state.progress.loading);
   const colorTheme = useSelector((state) => state.settings.colorTheme);
   const exerciseData = _.get(
      exerciseRecords,
      exerciseState.selectedExerciseId,
      []
   );
         ...
         <TwoColumnView>
            <ExerciseName
               longText={
                  ...
                  exerciseState.selectedExerciseName.length >= 20
               }
            >
               {exerciseState.selectedExerciseName || 'Select an Exercise'}
            </ExerciseName>
            <SelectExerciseButton>
               <Buttontext
                  onPress={() => {
                     navigation.navigate('Select Chart Exercise', {
                        onExerciseSelect: setSelectedExercise,
                        selectedExerciseId: exerciseState.selectedExerciseId,
                     });
                  }}
               >
                  Select Exercise
         ...
         {loading ? (
            ...
            <LineChart
               ...
               data={{
                  ...
                  datasets: [
                     {
                        data: exerciseData.map(
                           (exerciseInfo) => exerciseInfo.weight
                        ),
                     },
                  ],
               }}
            ...
            <CenterText>
               <StyledText>
                  No Data for {exerciseState.selectedExerciseName}.
               </StyledText>
               <StyledText>
                  Start logging workouts with{' '}
                  {exerciseState.selectedExerciseName} to see progress here!
               </StyledText>
            </CenterText>
         ) : (
            <CenterText>
