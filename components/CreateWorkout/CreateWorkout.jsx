...
const getExercise = (exercises, exerciseId) =>
   _.find(exercises, (exercise) => exercise.id === exerciseId);
...
const CreateWorkout = ({ navigation, route }) => {
   ...
   const [itemState, setItemState] = React.useState(
      editWorkout ? editWorkout.exercises.map(parseWorkoutExercises) : []
   );
   ...
   const onExercisesAdd = (selectedExercises) => {
      ...
      newItems.push(
         ...selectedExercises.map((exercise) => ({ ...exercise, isReps: true }))
      );
   ...
   return (
      <View style={{ height: '100%' }}>
         ...
         <AddFinishButton
            onPress={() => {
               if (!name) {
                  ...
                  if (isEditing) {
                     dispatch(
                        actions.workouts.updateWorkout(editWorkout.id, {
                           ...parsedItemState,
                           name,
                        })
                     );
         ...
         <AddCycleButton
            ...
            size={18}
            onPress={() => {
               navigation.navigate('Add Exercises', { onExercisesAdd });
            }}
         />
      </View>
   );
